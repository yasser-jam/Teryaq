...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { mutate: createType, isPending: isCreating } = useMutation({
    mutationFn: (data: FormData) => api('/types', { method: 'POST', body: data }),
    onSuccess: () => {
      goBack();
    }
  })
  ...
  const { mutate: updateType, isPending: isUpdating } = useMutation({
    mutationFn: (data: FormData) => api(`/types/${id}`, { method: 'PUT', body: data }),
    onSuccess: () => {
      goBack();
    }
  })
  ...
  return (
    <>
      <BasePageDialog
        ...
        subtitle={id === 'create' ? 'Add a new type' : 'Update type details'}
        className='w-[500px]'
        footer={actions}
        onOpenChange={goBack}
      >
